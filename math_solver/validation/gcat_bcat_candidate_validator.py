...
def validate_file(path: Path) -> Dict[str, Any]:
    try:
        ...
        return {
            ...
            "expected": None,
            "actual": FAIL_CLOSED,
            "reason_expected": None,
            "reason_actual": "malformed_json",
            "error": str(exc),
            "metrics": {},
...
def build_markdown_summary(report: Dict[str, Any]) -> str:
    ...
    for r in report["results"]:
        ...
        lines.append(
            f"| {r.get('id')} | {r.get('actual')} | {r.get('reason_actual')} | "
            f"{'✅' if r.get('passed') else '❌'} | "
            f"{c.get('total_cost', 0.0):.6f} | {c.get('budget', 0.0):.6f} | {c.get('budget_margin', 0.0):.6f} |"
        )
...
def main() -> int:
    ...
    report = {
        "schema": "stegverse.gcat_bcat.candidate_validation_report.v3",
        "summary": {
            "total": len(results),
