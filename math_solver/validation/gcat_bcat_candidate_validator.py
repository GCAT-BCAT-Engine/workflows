...
def main() -> int:
    ...
    report = {"schema": "stegverse.gcat_bcat.candidate_validation_report.v3", "summary": {"total": len(results), "passed": sum(1 for r in results if r["passed"]), "failed": sum(1 for r in results if not r["passed"])}, "cost_summary": build_cost_summary(results), "results": results}
