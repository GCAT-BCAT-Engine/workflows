...
def build_replay_vector(receipt: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(basis, dict):
        return {
            "id": receipt.get("candidate_id"),
            "name": receipt.get("candidate_id"),
            "params": basis.get("params"),
...
def replay_receipt(receipt: Dict[str, Any], expected_prev: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "prev_hash_valid": prev_valid,
        "basis_hash_valid": basis_hash_valid,
        ...
        "replay_passed": hash_valid and prev_valid and basis_hash_valid and outcome_matches and reason_matches,
...
def build_summary(report: Dict[str, Any]) -> str:
    ...
    lines = [
        "# GCAT/BCAT Receipt Replay Summary", "",
        f"- Receipts replayed: **{summary['total']}**",
        f"- Replay passed: **{summary['passed']}**",
        f"- Replay failed: **{summary['failed']}**",
        f"- Chain valid: **{report['chain_valid']}**", "",
        "## Replay Results", "",
        "| ID | Original | Replay | Reason Match | Hash Valid | Basis Hash | Prev Valid | Passed |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for r in report["results"]:
        lines.append(
            f"| {r['candidate_id']} | {r['original_outcome']} | {r['replay_outcome']} | "
            f"{'✅' if r['reason_matches'] else '❌'} | {'✅' if r['hash_valid'] else '❌'} | "
            f"{'✅' if r['basis_hash_valid'] else '❌'} | {'✅' if r['prev_hash_valid'] else '❌'} | "
            f"{'✅' if r['replay_passed'] else '❌'} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def run_replay(receipts_path: Path, out_dir: Path) -> Dict[str, Any]:
    ...
    report = {
        "schema": "stegverse.sandbox.gcat_bcat.replay_report.v2",
