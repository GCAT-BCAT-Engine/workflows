#!/usr/bin/env python3
"""
GCAT/BCAT StegVerse Sandbox Adapter
Version: 0.1.0

Wraps deterministic GCAT/BCAT candidate validation in a StegVerse-style sandbox layer:
- ingestion hash
- basis hash
- decision receipt
- receipt hash
...
def build_receipt(vector_path: Path, result: Dict[str, Any], vector: Dict[str, Any], prev_hash: str) -> Dict[str, Any]:
    payload = {
        "schema": "stegverse.sandbox.gcat_bcat.receipt.v1",
        ...
        "validator": {
            "name": "gcat_bcat_candidate_validator.py",
            "version": "0.3.0",
        },
...
def verify_chain(receipts: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "schema": "stegverse.sandbox.receipt_chain_verification.v1",
...
def build_summary(report: Dict[str, Any], chain_report: Dict[str, Any]) -> str:
    ...
    receipt_by_id = {r["candidate_id"]: r for r in report["receipts"]}
    for result in report["results"]:
        receipt = receipt_by_id.get(result.get("id"), {})
        lines.append(
            f"| {result.get('id')} | {result.get('actual')} | {result.get('reason_actual')} | "
            f"{'✅' if result.get('passed') else '❌'} | `{receipt.get('receipt_hash', '')[:16]}` |"
        )
...
def run_sandbox(vectors: Path, out_dir: Path) -> Dict[str, Any]:
    ...
    report = {
        "schema": "stegverse.sandbox.gcat_bcat.report.v1",
        "summary": {
            "total": len(results),
    ...
    (out_dir / "gcat_bcat_sandbox_report.json").write_text(
    ...
    (out_dir / "gcat_bcat_sandbox_chain_report.json").write_text(
        json.dumps(chain_report, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    (out_dir / "gcat_bcat_sandbox_summary.md").write_text(
        build_summary(report, chain_report),
        encoding="utf-8",
    )
