"""
pe_receipt_replay.py
---------------------

Replay Probability of Existence (PE) receipts and verify their integrity.
This script re-evaluates each candidate referenced by a receipt using
``validate_pe_candidate`` from ``pe_validator.py`` and checks that the
...
the stored outcome/reason and the replayed outcome/reason is flagged as
a mismatch.  A report summarising the replay and a Markdown summary are
generated.

Usage::

    python -m ecat_icat.pe_receipt_replay \
      --receipts ecat_icat/brain_reports/pe_receipts.jsonl \
      --vectors  ecat_icat/candidate_vectors/pe \
      --report   ecat_icat/brain_reports/pe_replay_report.json \
      --summary  ecat_icat/brain_reports/pe_replay_summary.md

The JSON report includes the number of matches/mismatches and details
for any mismatched receipts.  The Markdown summary provides a table
...
def replay_receipts(receipts_path: str, vectors_dir: str):
    """
    Replay receipts by re-running the PE validator for each candidate.
    ...
    for rec in receipts:
        ...
        if not path:
            ...
            cand = load_pe(path)
            replay_outcome, replay_reason, _ = validate_pe_candidate(cand)
...
def save_report_and_summary(replay_results: List[Dict], chain_valid: bool, mismatches: List[Dict], report_path: str, summary_path: str) -> None:
    ...
    report = {
        'schema': 'stegverse.sandbox.pe.replay_report.v1',
        'total': total,
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Replay PE receipts and verify outcomes.')
    parser.add_argument('--receipts', required=True, help='Path to receipts JSONL file')
    parser.add_argument('--vectors', required=True, help='Path to PE candidate directory')
